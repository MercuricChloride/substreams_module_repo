...
pub fn gas_guzzlers(blk: eth::Block) -> Result<Hotdogs, SubstreamError> {
    ...
    let hotdogs: Vec<Hotdog> = blk
        ...
        .filter_map(|transaction| {
            ...
            output_map.insert("block_hash".to_string(), ValueEnum::StringValue(block_hash.clone()));
            output_map.insert("block_number".to_string(), ValueEnum::Uint64Value(block_number));
            output_map.insert("block_timestamp".to_string(), ValueEnum::Uint64Value(block_timestamp.parse::<u64>().unwrap()));
